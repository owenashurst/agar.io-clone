...
/***/ function(module, exports, __webpack_require__) {
	...
	function toggleDarkMode() {
	    ...
	    if (backgroundColor === LIGHT) {
	        ...
	        lineColor = LINEDARK;
	        chat.addSystemLine('Darkness enabled.');
	    } else {
	        backgroundColor = LIGHT;
	        lineColor = LINELIGHT;
	        chat.addSystemLine('Darkness disabled.');
	    }
	}
	...
	chat.registerCommand('dark', 'Toggle darkness.', function () {
	    toggleDarkMode();
	});
	...
	chat.registerCommand('roundfood', 'Toggle drawing of food.', function (args) {
	    toggleRoundFood(args);
	});

	chat.registerCommand('help', 'List of commands available.', function () {
	    chat.printHelp();
	});
	...
	function setupSocket(socket) {
	    ...
	    socket.on('welcome', function (playerSettings) {
	        ...
	        gameStart = true;
	        debug('Game started at the following time: ' + gameStart);
	        chat.addSystemLine('You have successfully connected to OSMOSIS.');
	    ...
	    socket.on('playerDied', function (data) {
	        chat.addSystemLine('{OSMOSIS} - <b>' + (data.name.length < 1 ? 'An unnamed cell' : data.name) + '</b> was eaten.');
	    });

	    socket.on('playerDisconnect', function (data) {
	        chat.addSystemLine('{OSMOSIS} - <b>' + (data.name.length < 1 ? 'An unnamed cell' : data.name) + '</b> disconnected from the game.');
	    });

	    socket.on('playerJoin', function (data) {
	        chat.addSystemLine('{OSMOSIS} - <b>' + (data.name.length < 1 ? 'An unnamed cell' : data.name) + '</b> joined the game.');
	    });
