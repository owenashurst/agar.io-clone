...
function addFoods(target) {
    foods.push({
        ...
        // Trying to optimize, so no random color (temporarily remove the rotation change too)
        //color: randomColor(),
...
io.on('connection', function (socket) {
    ...
    var currentPlayer = {};
    var userID = startingId++;

    var playerSettings = new ArrayBuffer(1 + 1 + 4); // Message + hue(reduced to 1 byte) + unique id
    var view = new DataView(playerSettings);
    view.setUint8(0, 0); // Save a Welcome message in the array
    view.setUint8(1, Math.round(Math.random() * 255)); // Set the hue
    view.setUint32(2, userID); // Set the uid

    socket.emit(0, playerSettings); // Emit a Welcome message

    // 1 -> gotit
    socket.on(1, function (data) {
        var receivedPlayerUid = new DataView(data);
        var player = {
            id: receivedPlayerUid.getUint32(1)
        };
        ...
        var arrayLengthInBytes = users.length * 4; // Every uid is represented by a 32-bit unsigned integer, that's 4 bytes for each one of them
        var playersArray = new ArrayBuffer(1 + 1 + arrayLengthInBytes); // Message + array length + array of uids
        var view = new DataView(playersArray);
        view.setUint8(0, 2); // Save a playerJoin message in the array
        view.setUint8(1, users.length); // Save the length of the list in the array
        for (var i = 2; i < users.length; i++) {
            view.setUint32(i, users[i - 2]);
        }

        // Why not socket.emit?
        io.emit(2, playersArray); // Emit a playerJoin message
